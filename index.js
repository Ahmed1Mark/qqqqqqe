...
client.on('messageCreate', async (message) => {
  ...
  if (message.content.toLowerCase() === 'test') {
    const buttonRow = new MessageActionRow()
      .addComponents(
        new MessageButton()
          .setCustomId('send-msg-button')
          .setStyle('SECONDARY')
          .setLabel('Open modal dialog')
      );

    message.reply({
      content: 'You typed "test"',
...
client.on('interactionCreate', async (interaction) => {
  if (interaction.isButton()) {
    if (interaction.customId === 'send-msg-button') {
  ...
  if (interaction.isModalSubmit()) {
    if (interaction.customId === 'send-msg-modal') {
      ...
      const response3 = interaction.fields.getTextInputValue('3send-msg-input');
      ...
      interaction.reply({ content: `Message Has Been Sent Successfully`, ephemeral: true })
        .then(() => {
          ...
          const embed = new MessageEmbed()
            ...
            .setDescription(`${response} ${response2} ${response3}`);
...
const embed122s = new MessageEmbed()
    .setTitle("> <a:3_:1089615585232556204> Welcome To Server __Ra'ad__ Community <a:12:1150947511146664017>")
